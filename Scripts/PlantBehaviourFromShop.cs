...
public class PlantBehaviourFromShop : MonoBehaviour
{
    private int plantID;
    ...
    void Start()
    {
        ...
        IDataReader reader;
        string query = "SELECT * FROM plants WHERE id = '"+plantID+"'";
        cmnd_read.CommandText = query;
        reader = cmnd_read.ExecuteReader();

        while (reader.Read())
        {
            plantStats[0] = reader[0].ToString(); //id
            plantStats[1] = reader[1].ToString(); //type
            plantStats[2] = reader[2].ToString(); //stage
            plantStats[3] = reader[3].ToString(); //happiness
            plantStats[4] = reader[4].ToString(); //stage_timer
            plantStats[5] = reader[5].ToString(); //happiness_timer
            plantStats[6] = reader[6].ToString(); //happy_reduce
        }
